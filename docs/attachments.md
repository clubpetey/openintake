...
| **linear** | For each attachment: `POST /upload/file` (GraphQL `fileUpload` mutation via REST) → upload raw bytes to the returned signed URL → receive asset URL. Then `issueCreate` references the asset URLs via `attachmentLinks`. Upload BEFORE create — failure returns error before `issueCreate`, so no orphan issue. The upload response's `success` field is checked explicitly (L023 contract); a `success:false` response rejects before issueCreate. | N additional roundtrips per request (one per attachment). |
...
The published `capabilities.attachments.allowed_mime_types` returned by `/init` is the **union** across all enabled adapters' `Capabilities().AcceptedMIMETypes` **intersected** with `cfg.Attachments.AllowedMIMETypes`. The widget restricts capture/file-picker to this list, and the relay rejects out-of-list MIME types at validation time with `415 attachment_mime_not_allowed` — so by the time an attachment reaches an adapter, its MIME is guaranteed to be in the published allowlist. In v0 all five adapters advertise the same set (PNG/JPEG/WebP), so the math collapses and every routed attachment is acceptable to every adapter. Operators wanting stricter per-adapter enforcement should configure `allowed_mime_types` to the intersection by hand.
