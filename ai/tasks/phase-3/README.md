...
- **Live-smoke maintainer-pause path — ✅ COMPLETE for steps 1, 2, 4, 5, 6** (2026-05-27, on `main`). Maintainer ran `intake-license keygen`, embedded the printed `EmbeddedPublicKeyBase64` in `relay/internal/license/embedded_key.go`, and signed a short-lived zendesk+linear test license via `intake-license sign`. CLI `verify` accepted it with the embedded pubkey (the cross-module canonicalization lock — design §2.3). Two live relay boots then confirmed the gate end-to-end:
  - **Step 5 (licensed → gate permits paid):** `INTAKE_LICENSE_FILE` pointing at the signed license, zendesk enabled. Startup log: `relay: license  mode=licensed  detail="licensed: tier=pro, 14 day(s) remaining (expires 2026-06-11)"`, then `adapter enabled adapter=zendesk` — gate let the paid adapter through.
  - **Step 4 (free → gate blocks paid + rule dropped):** no license file, seeded an expired `state.json` (`trial_started_at` 30 days ago) at `%AppData%\intake\state.json` to force free mode, added a routing rule `bug→zendesk` to the YAML, intentionally left `ZENDESK_API_TOKEN` unset. Startup logs (in order): `relay: license  mode=free  detail="trial expired 2026-05-12 — FREE mode (paid adapters disabled) ..."` → `WARN adapter "zendesk" requires a license — disabled` → `WARN router: dropping rule targeting unregistered adapter  to=zendesk` → `router ready  default_adapter=webhook  adapters=["webhook"]`. Three invariants in one boot: expired-trial-downgrade-not-fatal, gate-before-token (no fatal missing-token even with `ZENDESK_API_TOKEN` unset), routing-rule-dangling-drop-with-warn. Step 6's "free-mode log" assertion is a subset of step 4 — covered by the same boot.
  - **Teardown** ran clean: seeded state file + temp configs/license removed; private key remains offline (`local-dev/` is gitignored).
- **Step 3 (live Chatwoot ticket creation) — ✅ COMPLETE** (2026-05-27, against Chatwoot Cloud `app.chatwoot.com`, account 167494, API-channel inbox 111666). `drive.ts` drove init → 2 turns → submit; the relay routed to the chatwoot adapter; a real conversation was created. The smoke surfaced the spec §5.1 wrinkle: the agent-side `POST /conversations` returns `404 "Resource could not be found"` (not 422) when the supplied `source_id` doesn't map to an existing contact_inbox. **Adapter patched to a two-call flow:** `POST /contacts` first (returns `contact_inbox.source_id`), then `POST /conversations` with that `source_id` + the new `contact_id`. New unit test `TestChatwootCreate_ContactCreateFails` covers the contact-step error path. Re-smoke: `[smoke] PASS`, conversation visible in the Chatwoot UI.
- **Live Linear issue creation — ✅ COMPLETE** (2026-05-28 fast-follow, against `api.linear.app`, RefSquare workspace, team key `REF`). `drive.ts` → submit → real issue created in the REF team (visible at the returned `external_url`). The smoke also surfaced a UX gap: Linear's GraphQL `IssueCreateInput.teamId` expects a team UUID (`9ddb7234-31d1-4dd3-b9b0-32ad948b6104`-style) that isn't in any Linear URL or UI label, while the short team key (`REF`) is visible everywhere. **Adapter enhanced to accept either form:** `team_id` is treated as a UUID if it matches the canonical regex, otherwise resolved to a UUID via one `query { teams(first: 250) { nodes { id name key } } }` call in `Configure` (10s timeout, fatal-at-startup if not found or unreachable). Six new unit tests cover UUID-passthrough (no HTTP call), key-resolved happy path, key-not-found (error names available keys), GraphQL-errors response, non-2xx, and transport error. Pattern is Linear-only by design — see L012. Re-smoke with `team_id: "REF"`: `[smoke] PASS`, REF-N issue visible in Linear.
- **Live Zendesk ticket creation — ✅ COMPLETE** (2026-05-28 fast-follow, against `mantichor.zendesk.com` trial, agent `pete@mantichor.com`). `drive.ts` → submit → real ticket #2 created (`external_id=2`, agent-UI URL `/agent/tickets/2`). **No adapter changes needed** — the basic-auth `{email}/token:{api_token}` base64 header, the `{ticket:{subject,comment:{body},priority,tags}}` body shape, the `severity_guess → priority` mapping, and the agent-UI URL construction all worked first-attempt against live Zendesk. The status-code-only non-2xx error format (L011 — auth header is base64-embedded; can't safely echo the body) didn't bite us because the smoke didn't fail.
- **Live Fider post creation — ✅ COMPLETE** (2026-05-28 fast-follow, against a self-hosted Fider Docker stack at `http://192.168.1.102:3000`, run on a separate LAN host). `drive.ts` → submit → real post created, `external_url` resolving correctly. **No adapter changes needed** — the Bearer auth, the `{title, description}` body, the `{id, number}` response parsing, and the `/posts/<number>` URL construction all worked first-attempt. The cross-machine smoke (relay on Windows, Fider on LAN Linux box) also incidentally confirmed `base_url` cross-LAN reachability and that nothing in the adapter assumes a same-host topology. Setup note: Fider has no hosted cloud, so a self-host stack is required; in addition to Fider + Postgres the stack needs a SMTP sink (MailHog) because Fider sends a verification email at admin signup. Required Fider envs for a minimal smoke deployment: `DATABASE_URL`, `JWT_SECRET`, `BASE_URL`, `EMAIL_NOREPLY`, `EMAIL_SMTP_HOST/PORT/USERNAME/PASSWORD/ENABLE_STARTTLS`, `BLOB_STORAGE=sql`.
- **Phase 3 live-adapter coverage: 4/4 ✅** (chatwoot, fider, zendesk, linear). All four adapters proven end-to-end against real downstream APIs.
