...
- **Step 3 (live Chatwoot ticket creation) — ⏸ OPTIONAL, deferred.** A downstream-API smoke separate from the gate critical path. Requires a running Chatwoot instance + a real `CHATWOOT_TOKEN`. Runs anytime the maintainer wants to validate the chatwoot adapter end-to-end against a real Chatwoot.
