...
- **Credit-free unit + integration layer — ✅ COMPLETE.** All four sub-plans implemented on `phase-4`, each through a two-stage (spec + code-quality) subagent review with fix-loops. `go build/vet/test ./...` green in `relay/` + `license-tool/`; `scripts/verify-contract.sh` + `scripts/check-pins.sh` green; `go mod tidy` adds exactly the two expected deps (`golang-jwt/jwt/v5 v5.3.1` + `MicahParks/keyfunc/v3 v3.8.0`), both exact-pinned. Covered credit-free: emailcode TTL/rate-limit/single-use with injected clock; emailjwt mint+verify + tamper/wrong-secret/expired/wrong-issuer/alg-confusion rejection; FakeSender captures; /auth/email/start happy/400/429/502 paths; /auth/email/verify happy/400/401 paths; integration spy proving SessionContext.AuthMode=="email"; SSO RS256+HS256 happy/tamper/wrong-iss/wrong-aud/expired/alg-confusion-both-directions/claim-mapping/skew-boundary/M2M-shape/empty-sub; factory both-set/neither-set errors. Dispatcher table-tested across all (anonymous, email, sso, none) × (request shape) combinations. Final whole-implementation review: READY TO MERGE (security boundary verified — alg-pinning at all three JWT parse sites + iss="intake-email" defense-in-depth + secret hygiene + frozen seams untouched).

- **Live email smoke — ✅ COMPLETE** (2026-05-28, against MailHog at `192.168.1.102:1025/8025`, smoke email `pete@mantichor.com`). `drive-auth-email.ts` drove init → /auth/email/start → MailHog captured the 6-digit code → /auth/email/verify minted a 15-min HS256 JWT → 2 turns + /submit with the bearer → webhook receiver captured the canonical payload showing `user.{auth_mode:"email", email:"pete@mantichor.com", verified:true}`. The smoke surfaced **one latent bug** in 4-i: the bearer-success branches of the dispatcher built `SessionContext` without `SessionID`, so `payloadbuild` emitted `IntakePayload.client.session_id=""` and the schema gate (L003 mitigation) correctly rejected the payload. **Patched at `8f79c76`**: dispatcher now reads `X-Intake-Session` (which `IntakeClient` already sends unconditionally) and populates `SessionContext.SessionID` in both bearer branches — matching the anonymous path's behavior. Two regression tests pin it. Adjacent: `core/src/client.ts` now surfaces the response body in submit-failure error messages (debug improvement kept permanent). See L015 for the lesson.

- **Live SSO smoke — ✅ COMPLETE** (2026-05-28, against real Auth0 tenant `refsquare-staging.us.auth0.com`, API audience `https://intake-smoke`, M2M client_credentials grant). `drive-auth-sso.ts` (new) drove init → 2 turns + /submit with the bearer minted directly from `https://refsquare-staging.us.auth0.com/oauth/token` → webhook receiver captured the canonical payload showing `user.{auth_mode:"sso", id:"<m2m-client>@clients", verified:true}`. The relay's `RS256Verifier` performed live JWKS fetch + alg pinning (RS256-only) + iss/aud/exp validation + claim mapping (`sub` → `UserID`). The M2M-shape (sub-only, no email/name claims) is precisely the case the 4-iii unit test `TestRS256_M2MShape_OnlySubPopulated` pinned — proven live. **Note on email/name mapping:** Auth0 M2M tokens (client_credentials grant) intentionally omit `email`/`name` claims since they represent an application, not a user — the canonical payload correctly emits `user.email`/`user.display_name` as absent. The full claim mapping path (sub+email+name → UserID+Email+DisplayName) is exhaustively unit-tested in 4-iii's RS256 happy path; the consumer-side deployment story (host app uses a real user-login flow that DOES carry those claims) was not separately live-smoked because the verifier code path is identical to the M2M path with different input — see "If you want to also smoke the human-user shape" in `ai/tasks/phase-4/4-iv-smoke-docs-plan.md` Task 3 for the password-grant alternative.

- **Phase 4 coverage: 3/3 modes proven** — anonymous (Phase 1, unchanged + regression-tested via `TestDispatcher_AnonymousFallthrough_Preserved`), email (live), SSO (live). The Phase-1 anonymous walking-skeleton flow continues to pass against the dispatcher (no Authorization header → X-Intake-Session resolves to anonymous as before).
