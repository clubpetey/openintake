...
- Phase 7 packaging: `@intake/vue` type declarations currently emit to `dist/src/` (vue-tsc `-b` mirrors `src/`) and `vue-tsc` emits dead `.js` alongside `.d.ts`. Before npm publish, configure `emitDeclarationOnly` + `rootDir: src` (or a separate `tsc --emitDeclarationOnly` step) so types land at `dist/index.d.ts` cleanly. `dist/` is gitignored, so this is build-output debt only.
- Phase 7 packaging: `@intake/core` is bundled into the `@intake/vue` ESM output (only `vue` is externalized). Before publish, externalize `@intake/core` and move it to `peerDependencies` to avoid a dual-copy in consumer bundles.
